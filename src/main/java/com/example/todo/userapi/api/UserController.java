...
public class UserController {
    ...
    public ResponseEntity<?> signUp(
        ...
        try {
            ...
            if(profileImg!=null) {
                ...
                // 전달받은 프로필 이미지를 먼저 지정된 로컬 경로에 저장한 후 DB저장을 위해 경로를 받아오자.
    ...
    public ResponseEntity<?> loadFile(@AuthenticationPrincipal TokenUserInfo userInfo) {
        log.info("/api/auth/load-profile - GET! : {}",userInfo.getEmail());

        try {
            // 클라이언트가 요청한 프로필 사진을 응답해야 함.
            // 1. 프로필 사진의 경로부터 얻어야한다.
            String filePath = userService.findProfilePath(userInfo.getUserId());

            // 2. 얻어낸 파일 경로를 통해 실제 파일 데이터를 로드하기.
            File profileFile = new File(filePath);

            // 프사가 없는 사람들은 경로가 존재하지 않음 ( not found )
            // 404 status를 리턴
            if(!profileFile.exists()) {
                return ResponseEntity.notFound().build();
            }

            // 해당 경로에 저장된 파일을 byte[] 배열로 직렬화 해서 리턴
            // 왜 배열로 ? , 파일의 손상을 최소화 해줌
    ...
    private MediaType findExtensionAndGetMediaType(String filePath) {
        ...
        // 파일 경로에서 확장자 추출하기
        // C:/todo_upload/adfasdfads_abc.jpg
        String ext
